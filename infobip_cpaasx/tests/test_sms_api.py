...
from infobip_cpaasx import ApiClient, Configuration, SmsAdvancedTextualRequest, SmsTextualMessage, SmsDestination, \
    ...
    SmsBulkRequest, SmsUpdateStatusRequest, SmsDeliveryResult, SmsReport, SmsError, SmsPrice, SmsLogsResponse, SmsLog, \
    SmsInboundMessageResult, \
    SmsInboundMessage, SmsDeliveryTimeWindow, SmsDeliveryDay, SmsDeliveryTimeFrom, SmsDeliveryTimeTo, SmsLanguage, \
    SmsUrlOptions, SmsSendingSpeedLimit, SmsSpeedLimitTimeUnit, SmsTracking, SmsRegionalOptions, SmsIndiaDltOptions, \
    SmsTurkeyIysOptions, SmsAdvancedBinaryRequest, SmsBinaryMessage, SmsBinaryContent, SmsPreviewRequest, \
    SmsPreviewResponse, SmsPreview, SmsLanguageConfiguration
...
def test_send_fully_featured_sms(httpserver: HTTPServer, sms_api_client):
    ...
    sms_advanced_textual_request = SmsAdvancedTextualRequest(
        ...
        messages=[
            SmsTextualMessage(
                ...
                send_at=datetime.datetime(2023, 8, 1, 16, 10, 0,
        ...
        sending_speed_limit=SmsSendingSpeedLimit(
            amount=sending_speed_limit_amount,
            time_unit=SmsSpeedLimitTimeUnit.HOUR
        ),
    ...
    actual_response = sms_api_client.send_sms_message(sms_advanced_textual_request=sms_advanced_textual_request)
...
def test_send_fully_featured_binary_sms(httpserver: HTTPServer, sms_api_client):
    ...
    actual_response = sms_api_client.send_binary_sms_message(sms_advanced_binary_request=sms_advanced_binary_request)
...
def test_preview_sms(httpserver: HTTPServer, sms_api_client):
    ...
    actual_response = sms_api_client.preview_sms_message(sms_preview_request=sms_preview_request)
...
def test_outbound_delivery_reports(httpserver: HTTPServer, sms_api_client):
    ...
    actual_response: SmsDeliveryResult = sms_api_client.get_outbound_sms_message_delivery_reports(
        bulk_id=bulk_id
    )
...
def test_outbound_logs(httpserver: HTTPServer, sms_api_client):
    ...
    query_string = to_query_string_without_escaping({
        "bulkId": bulk_id,
        "sentSince": "2023-03-09T14%3A00%3A00%2B00%3A00"
    })
    ...
    actual_response: SmsLogsResponse = sms_api_client.get_outbound_sms_message_logs(
        bulk_id=[bulk_id],
        sent_since=datetime.datetime(2023, 3, 9, 14, 0, 0, 0, tzinfo=datetime.timezone.utc)
    )
